(()=>{
  ...
  const PAYLOAD_SCHEMA=4, PUBLISH_REV='v288-r1', PART_CHARS=150000, OFFLINE_TTL=24*60*60*1000;
  ...
  window.priceBookPayload287=bookPayload;
  function accountPayload287(u){
    if(u?.role==='sales'||u?.ledgerAccess!==true||!u?.ledgerPartyId)return null;
    const party=(S.parties||[]).find(x=>x.id===u.ledgerPartyId);if(!party)return null;
    const payments=(S.payments||[]).filter(x=>x.partyId===party.id).map(x=>({id:x.id,date:+x.date||0,amount:+x.amount||0,mode:safe(x.mode||''),ref:safe(x.ref||''),note:safe(x.note||''),allocations:(x.allocations||[]).map(a=>({invId:a.invId,amt:+a.amt||0}))}));
    const paid=id=>round2(payments.reduce((sum,p)=>sum+(p.allocations||[]).filter(a=>a.invId===id).reduce((s,a)=>s+(+a.amt||0),0),0));
    const invoices=(S.invoices||[]).filter(x=>x.partyId===party.id&&!x.cancelled).map(x=>{const got=paid(x.id),total=+x.total||0;return{id:x.id,no:safe(x.no||''),date:+x.date||0,items:(x.items||[]).map(i=>({code:safe(i.code||''),name:safe(i.name||''),size:safe(i.size||''),qty:+i.qty||0,rate:+i.rate||0,amt:+i.amt||0,gst:+i.gst||0,uom:safe(i.uom||'')})),subtotal:+x.subtotal||0,gstAmt:+x.gstAmt||0,roundOff:+x.roundOff||0,total,paid:got,due:round2(Math.max(0,total-got)),withGst:!!x.withGst,source:x.source==='customer-bill-scan'?'Uploaded bill':''}}).sort((a,b)=>b.date-a.date);
    const tx=[];invoices.forEach(i=>tx.push({date:i.date,type:'Bill',ref:i.no,detail:(i.items||[]).length+' items',debit:i.total,credit:0}));payments.forEach(p=>tx.push({date:p.date,type:'Payment',ref:[p.mode,p.ref].filter(Boolean).join(' '),detail:p.note||'Received',debit:0,credit:p.amount}));tx.sort((a,b)=>a.date-b.date);
    let balance=+party.openingBalance||0;const ledger=[{date:0,type:'Opening',ref:'—',detail:'Opening balance',debit:balance>0?balance:0,credit:balance<0?-balance:0,balance:round2(balance)}];tx.forEach(x=>{balance+=x.debit-x.credit;ledger.push({...x,balance:round2(balance)})});
    return{party:{id:party.id,name:safe(party.name||''),city:safe(party.city||''),phone:safe(party.phone||'')},summary:{opening:round2(+party.openingBalance||0),billed:round2(invoices.reduce((a,x)=>a+x.total,0)),received:round2(payments.reduce((a,x)=>a+x.amount,0)),balance:round2(balance)},invoices,payments,ledger};
  }
  ...
  function payloadFor(u){
    ...
    return {schema:PAYLOAD_SCHEMA,version:7,firm:S.settings.firm||'Business',currency:S.settings.currency||'₹',user:{name:u.name,role:u.role||'customer'},priceBooks:books,catalogs,account:accountPayload287(u),notice,announcement,accessExpiresAt:expires,updatedAt:now,offlineValidUntil:Math.min(now+OFFLINE_TTL,expires||Infinity),accessVersion};
  }
...
`;document.head.appendChild(st)
  ...
  function accountHtml287(p){const a=p.account;if(!a)return'';const c=p.currency||'₹',m=x=>Number(x||0).toLocaleString('en-IN',{maximumFractionDigits:2}),d=x=>x?new Date(x).toLocaleDateString('en-IN',{day:'2-digit',month:'short',year:'numeric'}):'—',s=a.summary||{};return `<div class="cv287-account-head"><div><span>Your account</span><h2>${esc(a.party?.name||'Account')}</h2><small>${esc(a.party?.city||'Live bill and payment statement')}</small></div><div class="cv287-balance"><span>Current balance</span><b>${c}${m(Math.abs(s.balance))}</b><small>${s.balance>=0?'Amount due':'Advance balance'}</small></div></div><div class="cv287-kpis"><div><span>Total billed</span><b>${c}${m(s.billed)}</b></div><div><span>Payments received</span><b>${c}${m(s.received)}</b></div><div><span>Open bills</span><b>${(a.invoices||[]).filter(x=>x.due>.5).length}</b></div></div><div class="cv287-ledger"><div class="cv272-section-title"><b>Account statement</b><span>${(a.ledger||[]).length} entries</span></div><div class="tbl-wrap"><table><thead><tr><th>Date</th><th>Entry</th><th>Reference</th><th class="r">Billed</th><th class="r">Received</th><th class="r">Balance</th></tr></thead><tbody>${(a.ledger||[]).slice().reverse().map(x=>`<tr><td>${d(x.date)}</td><td><b>${esc(x.type)}</b><div class="note">${esc(x.detail||'')}</div></td><td>${esc(x.ref||'—')}</td><td class="r">${x.debit?c+m(x.debit):'—'}</td><td class="r good">${x.credit?c+m(x.credit):'—'}</td><td class="r"><b>${c}${m(Math.abs(x.balance))} ${x.balance>=0?'Dr':'Cr'}</b></td></tr>`).join('')||'<tr><td colspan="6" class="empty-mini">No account entries yet.</td></tr>'}</tbody></table></div></div><div class="cv272-section-title" style="margin-top:18px"><b>Bills</b><span>${(a.invoices||[]).length} posted</span></div><div class="cv272-listgrid">${(a.invoices||[]).map(i=>`<article class="cv272-book-card"><span class="cv272-tag ${i.due>.5?'wholesale':'retail'}">${i.due>.5?'Due '+c+m(i.due):'Paid'}</span><h3>${esc(i.no)}</h3><div class="cv272-card-meta">${d(i.date)} · ${(i.items||[]).length} products · Total ${c}${m(i.total)}</div><details><summary>View products</summary><div class="tbl-wrap" style="margin-top:8px"><table><tbody>${(i.items||[]).map(x=>`<tr><td><b>${esc(x.code||'')}</b> ${esc(x.name)}</td><td class="r">${m(x.qty)} × ${c}${m(x.rate)}</td><td class="r"><b>${c}${m(x.amt)}</b></td></tr>`).join('')}</tbody></table></div></details></article>`).join('')||'<div class="cv47-empty">No bills posted yet.</div>'}</div>`}
  function show(p,off=false){
    ...
    const books=activeBooks230(p),sales=p.user?.role==='sales',role=sales?'Sales team':'Customer',accountAssigned=p.user?.role==='customer'&&!!p.account?.party?.id,audCount=a=>books.filter(b=>audience230(b)===a).length;
    ...
    e.innerHTML=`<div class="cv272-shell"><div class="cv272-topbar"><div class="cv272-topbar-in"><div class="cv272-mark"><img src="ca-logo.png" alt="CA"></div><div class="cv272-brand"><b>CA</b><span>${esc(p.firm||'Business')} · ${esc(p.user?.name||'')} · ${role}${off?' · Offline':''}</span></div><div class="cv272-session"><span class="cv272-role">${role}</span><button class="btn ghost sm cv272-logout" id="cv46out">Log out</button></div></div></div><div class="cv47-wrap">
      ...
      ${accountAssigned?`<section id="cv272account" hidden>${accountHtml287(p)}</section>`:''}
    </div></div>`;
    ...
    e.dataset.activeTab='products';e.querySelectorAll('[data-tab272]').forEach(b=>b.onclick=()=>{e.dataset.activeTab=b.dataset.tab272;e.querySelectorAll('[data-tab272]').forEach(x=>{x.classList.toggle('on',x===b);x.setAttribute('aria-pressed',String(x===b))});['products','lists','catalogs','account'].forEach(k=>{const sec=e.querySelector('#cv272'+k);if(sec)sec.hidden=b.dataset.tab272!==k})});
