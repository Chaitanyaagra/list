(()=>{
  ...
  const PAYLOAD_SCHEMA=3, PUBLISH_REV='v267-r1', PART_CHARS=150000, OFFLINE_TTL=24*60*60*1000;
  let cloudSession=null, publishTimer=null, publishSuppress=false, restrictedSyncQueue=Promise.resolve(), cloudPublishDirty265=true;
  ...
  window.clearRestrictedOfflineCaches265=async()=>{try{Object.keys(localStorage).filter(k=>k.startsWith(CACHE_PREFIX)||k.startsWith('pm-viewer-prefs-v230:')||k.startsWith('pm-order-cart-233:')).forEach(k=>localStorage.removeItem(k))}catch(e){}try{await caches.delete('pm-viewer-catalogs-v228')}catch(e){}};
  ...
  setTimeout(()=>render(),0);setTimeout(()=>schedulePublish(500),900);window.payloadFor46=payloadFor;window.showViewerPreview46=(p)=>show(p,false);window.getCloudSession233=()=>cloudSession;window.PriceManagerAPI={...(window.PriceManagerAPI||{}),version:'2.68 refinement-batch-device-ui'};
})();
