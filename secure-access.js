...
#cloudViewer46 .cv47-search{position:sticky;top:63px!important;z-index:15;background:rgba(244,242,236,.92)!important;backdrop-filter:blur(16px);padding:8px 0 11px!important}.cv272-search-panel{background:var(--cv-panel);border:1px solid var(--cv-line);border-radius:18px;padding:12px;box-shadow:0 8px 26px rgba(28,34,39,.06)}#cloudViewer46 .cv47-searchbox{display:grid!important;grid-template-columns:minmax(0,1fr) 210px!important;gap:8px!important;background:transparent!important;border:0!important;padding:0!important;box-shadow:none!important}.cv272-search-main{position:relative}.cv272-search-main:before{content:'⌕';position:absolute;left:13px;top:8px;font-size:24px;color:#888c85}.cv272-search-main input{padding-left:41px!important;font-size:14px!important}.cv272-tools{display:flex;gap:7px;flex-wrap:wrap;margin-top:9px}.cv272-tools button{border:1px solid #ded8cd;background:#fff;border-radius:999px;padding:6px 10px;font-weight:700;color:#565c59;cursor:pointer}
...
`;document.head.appendChild(st)
  ...
  function renderResults(root,p,mode='search'){
    ...
    if(mode==='search'&&!q.trim()){
      ...
      box.innerHTML=`<div class="cv47-empty"><div style="font-size:25px;margin-bottom:7px">⌕</div><b>Search products in the selected price list</b><div style="margin-top:4px">Use product name, code, barcode, size, packing or UOM.</div>${chips?'<div class="cv272-tools" style="justify-content:center;margin-top:12px">'+chips+'</div>':''}</div>`;
  ...
  function show(p,off=false){
    ...
    e.innerHTML=`<div class="cv272-shell"><div class="cv272-topbar"><div class="cv272-topbar-in"><div class="cv272-mark">${esc(initials)}</div><div class="cv272-brand"><b>${esc(p.firm||'Price Manager')}</b><span>${esc(p.user?.name||'')} · ${role}${off?' · Offline':''}</span></div><div class="cv272-session"><span class="cv272-role">${role}</span><button class="btn ghost sm cv272-logout" id="cv46out">Log out</button></div></div></div><div class="cv47-wrap">
      ...
      <section id="cv272products"><div class="cv47-search"><div class="cv272-search-panel"><div class="cv47-searchbox"><div class="cv272-search-main"><input id="cv47q" type="search" autocomplete="off" placeholder="Search product, code, barcode, size, packing..."></div><select id="cv47catfilter"><option value="">All categories</option></select></div><div class="cv272-tools"><button id="cv47fav">★ Favorites</button><button id="cv47recent">Recent products</button></div></div><div id="cv47results" class="cv47-results"></div></div></section>
