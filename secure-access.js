(()=>{
  ...
  const PAYLOAD_SCHEMA=3, PUBLISH_REV='v230-r1', PART_CHARS=150000, OFFLINE_TTL=24*60*60*1000;
  let cloudSession=null, publishTimer=null, publishSuppress=false, restrictedSyncQueue=Promise.resolve(), lastPublishFingerprint='', pendingPublishFingerprint='';
  ...
  function payloadFor(u){
    ...
    return {schema:PAYLOAD_SCHEMA,version:5,firm:S.settings.firm||'Price & Stock Manager',currency:S.settings.currency||'₹',user:{name:u.name,role:u.role||'customer'},priceBooks:books,catalogs,notice,announcement,accessExpiresAt:expires,updatedAt:now,offlineValidUntil:Math.min(now+OFFLINE_TTL,expires||Infinity),accessVersion};
  }

  async function establishRestrictedAccount(cfg,u){
    const email=await loginEmail(u.name,cfg.projectId),pw=await loginPassword(u.name,u.pin,cfg.projectId);let sec=null,uid='';
    try{
      sec=await secondary(cfg,u.id||'user');
      const candidates=[];
      if(u.cloudUid&&u.cloudNameSnapshot&&u.cloudPinSnapshot)candidates.push([await loginEmail(u.cloudNameSnapshot,cfg.projectId),await loginPassword(u.cloudNameSnapshot,u.cloudPinSnapshot,cfg.projectId)]);
      candidates.push([email,pw]);
      let cred=null;
      for(const [em,pass] of candidates){try{cred=await sec.auth.signInWithEmailAndPassword(em,pass);break}catch(e){}}
      if(cred){uid=cred.user.uid;if(cred.user.email!==email)await cred.user.updateEmail(email);try{await cred.user.updatePassword(pw)}catch(e){}}
      else{
        try{cred=await sec.auth.createUserWithEmailAndPassword(email,pw);uid=cred.user.uid}
        catch(e){if(e.code==='auth/email-already-in-use'){cred=await sec.auth.signInWithEmailAndPassword(email,pw);uid=cred.user.uid}else throw e}
      }
      return {uid,email,sec};
  ...
  async function syncOneNow(id){
    ...
    const oldUid=u.cloudUid||'',acct=await establishRestrictedAccount(cfg,u),uid=acct.uid,email=acct.email,sec=acct.sec;
    try{if(oldUid&&oldUid!==uid){const oldProfile=db.collection('accessProfiles').doc(oldUid);await oldProfile.set({active:false,revokedAt:firebase.firestore.FieldValue.serverTimestamp()},{merge:true});await deletePayload(db,oldUid);await oldProfile.delete()}
      ...
      Object.assign(u,{cloudUid:uid,cloudEmail:email,cloudPinSnapshot:u.pin,cloudNameSnapshot:u.name,cloudSyncedAt:Date.now(),cloudAccessVersion:payload.accessVersion});publishSuppress=true;try{save()}finally{publishSuppress=false}return u;
    }finally{try{await sec.auth.signOut()}catch(e){}try{await sec.app.delete()}catch(e){}}
  }
  function syncOne(id){const run=()=>syncOneNow(id);restrictedSyncQueue=restrictedSyncQueue.then(run,run);return restrictedSyncQueue}
  async function revokeUser(u,{deleteAuth=true}={}){
    ...
    try{
      if(u.cloudUid){
        ...
        await pref.delete();
        if(deleteAuth){let sec=null;try{sec=await secondary(cfg,'revoke');const names=[[u.cloudNameSnapshot||u.name,u.cloudPinSnapshot||u.pin],[u.name,u.pin]];let cred=null;for(const [nm,pn] of names){try{cred=await sec.auth.signInWithEmailAndPassword(await loginEmail(nm,cfg.projectId),await loginPassword(nm,pn,cfg.projectId));break}catch(e){}}if(cred)await cred.user.delete()}catch(e){}finally{if(sec){try{await sec.auth.signOut()}catch(_){}try{await sec.app.delete()}catch(_){}}}}
      }
  ...
  window.syncOneUser46=async id=>{try{const u=await syncOne(id);toast('Firebase access updated for '+u.name);render()}catch(e){toast(e.message||'Could not sync user')}};
  window.syncAllUsers46=async()=>{try{if(!(S.viewerUsers||[]).length)return toast('Create a restricted user first');for(const u of S.viewerUsers)if(u.active!==false||u.cloudUid)await syncOne(u.id);toast('Published restricted users securely');render()}catch(e){toast(e.message||'Could not publish users')}};
  ...
  function activeAnnouncement230(a){if(!a?.enabled||!a.text)return false;const now=new Date().toISOString().slice(0,10);return(!a.from||a.from<=now)&&(!a.until||a.until>=now)}
  ...
  function renderResults(root,p,mode='search'){const q=root.querySelector('#cv47q')?.value||'',bookId=root.querySelector('#cv47book')?.value||'',category=root.querySelector('#cv47catfilter')?.value||'',box=root.querySelector('#cv47results');if(!box)return;if(mode==='search'&&!q.trim()){const pr=prefs230(p);const chips=pr.recentQueries.map(x=>`<button class="btn ghost sm" data-q230="${esc(x)}">${esc(x)}</button>`).join('');box.innerHTML=`<div class="cv47-empty"><div style="font-size:26px;margin-bottom:6px">🔍</div>Search by product, code, barcode, category, size, packing or UOM.${chips?'<div class="cv47-tools" style="justify-content:center">'+chips+'</div>':''}</div>`;box.querySelectorAll('[data-q230]').forEach(b=>b.onclick=()=>{root.querySelector('#cv47q').value=b.dataset.q230;renderResults(root,p)});return}const rows=searchRows(p,q,bookId,category,mode);if(mode==='search')rememberResults230(p,rows,q);const pr=prefs230(p);box.innerHTML=rows.length?rows.map((r,i)=>`<div class="card"><div class="bd cv47-row"><div><div class="cv47-booktag">${esc(r.b.name)} · ${esc(r.it.category||'')}</div><b>${esc(r.it.code)} · ${esc(r.it.name)}</b><button class="cv47-star" data-star230="${i}" title="Favorite">${pr.favorites.includes(r.key)?'★':'☆'}</button><div class="note">${esc(r.it.size||'')}${r.it.packing?' · '+esc(r.it.packing):''}${r.it.mrp!=null?' · MRP '+(p.currency||'₹')+Number(r.it.mrp).toLocaleString('en-IN',{maximumFractionDigits:2}):''}</div>${window.orderRowExtra233?window.orderRowExtra233(r,i):''}</div><div style="text-align:right"><div class="cv47-rate" data-rate47="${i}">${selectedRate(r.it,r.it.defaultUom,p)}</div>${!r.it.rateOnRequest&&(r.it.uomRates||[]).length>1?`<select class="cv47-uom" data-uom47="${i}">${r.it.uomRates.map(x=>`<option value="${esc(x.u)}" ${norm(x.u)===norm(r.it.defaultUom)?'selected':''}>${esc(x.u)}</option>`).join('')}</select>`:`<div class="metric-sub">${r.it.rateOnRequest?'Contact owner / sales team':esc(r.it.defaultUom||'')}</div>`}</div></div></div>`).join(''):'<div class="cv47-empty"><div style="font-size:26px;margin-bottom:6px">📭</div>No matching product in assigned price lists.<div class="note" style="margin-top:4px">Try a shorter search term, or check the category filter above.</div></div>';box.querySelectorAll('[data-uom47]').forEach(sel=>sel.onchange=()=>{const i=+sel.dataset.uom47,row=rows[i],rate=box.querySelector(`[data-rate47="${i}"]`);if(row&&rate)rate.textContent=selectedRate(row.it,sel.value,p)});box.querySelectorAll('[data-star230]').forEach(st=>st.onclick=()=>{const r=rows[+st.dataset.star230],x=prefs230(p);x.favorites=x.favorites.includes(r.key)?x.favorites.filter(k=>k!==r.key):[r.key,...x.favorites];savePrefs230(p,x);renderResults(root,p,mode)});if(window.wireOrderRows233)window.wireOrderRows233(box,rows,p)}
  ...
  function show(p,off=false){viewerCss();let e=document.getElementById('cloudViewer46');if(!e){e=document.createElement('div');e.id='cloudViewer46';e.style.cssText='position:fixed;inset:0;z-index:200000;background:#f6f2e9;overflow:auto';document.body.appendChild(e)}e.style.display='block';const books=activeBooks230(p),gs={};books.forEach(b=>(gs[b.category||'Other']=gs[b.category||'Other']||[]).push(b));const exp=p.offlineValidUntil?new Date(p.offlineValidUntil).toLocaleString('en-IN'):'',rn=p.notice||{},rd=noticeDate47(rn.effectiveDate),rtext=rn.text||('New rates implemented'+(rd?' from '+rd:'')),an=p.announcement||{},cats=[...new Set(books.flatMap(b=>(b.items||[]).map(x=>x.category).filter(Boolean)))].sort();e.innerHTML=`<div class="cv47-wrap"><div class="cv47-head"><div><b style="font-size:18px">${esc(p.firm||'Price List')}</b><div class="note">${p.user?.role==='sales'?'Sales team':'Customer'} · ${esc(p.user?.name||'')} ${off?'· Offline cached rates'+(exp?' valid until '+esc(exp):''):''}</div></div><button class="btn ghost sm" id="cv46out">Log out</button></div>${rn.enabled?`<div class="cv47-notice"><b>Rate update</b><div>${esc(rtext)}</div>${rd&&rn.text?`<div class="note" style="margin-top:3px">Effective from ${esc(rd)}</div>`:''}</div>`:''}${activeAnnouncement230(an)?`<div class="cv47-announce"><b>${esc(an.title||'Announcement')}</b><div>${esc(an.text)}</div></div>`:''}<div class="cv47-switch"><div class="card"><div class="bd"><b>Change price list</b><select id="cv47switchbook"><option value="">Show all assigned price lists</option>${books.map(b=>`<option value="${b.id}">${esc(b.name)}</option>`).join('')}</select></div></div><div class="card"><div class="bd"><b>Change / open catalog</b><select id="cv47switchcat"><option value="">Show all assigned catalogs</option>${(p.catalogs||[]).map(c=>`<option value="${c.id}">${esc(c.title||c.name||'Catalog')}</option>`).join('')}</select></div></div></div><div class="cv47-search"><div class="section-title">Quick product rate search</div><div class="cv47-searchbox"><input id="cv47q" type="search" autocomplete="off" placeholder="Search name, code, barcode, category, size, packing..."><select id="cv47book"><option value="">All assigned price lists</option>${books.map(b=>`<option value="${b.id}">${esc(b.name)}</option>`).join('')}</select><select id="cv47catfilter"><option value="">All categories</option>${cats.map(c=>`<option>${esc(c)}</option>`).join('')}</select></div><div class="cv47-tools"><button class="btn ghost sm" id="cv47fav">★ Favorites</button><button class="btn ghost sm" id="cv47recent">Recent products</button></div><div id="cv47results" class="cv47-results"><div class="cv47-empty">Search by product, code, barcode, category, size, packing or UOM.</div></div></div><div id="cv47priceSection"><div class="section-title" style="margin-top:14px">Assigned price lists</div>${Object.keys(gs).sort().map(c=>`<div class="metric-sub" style="font-weight:700;text-transform:uppercase;margin:14px 0 6px">${esc(c)}</div>${gs[c].map(b=>`<div class="card" data-book-card="${b.id}" style="margin-bottom:10px"><div class="bd"><b>${esc(b.name)}</b><div class="note">${(b.items||[]).length} searchable products${b.effectiveFrom?' · effective '+esc(noticeDate47(b.effectiveFrom)):''} · PDF download only</div><div class="cv47-actions"><button class="btn ghost sm" data-pdf="${b.id}">Download PDF</button><button class="btn primary sm" data-share="${b.id}">Share PDF / WhatsApp</button>${p.user?.role==='sales'&&((b.changes?.details||[]).length)?`<button class="btn ghost sm" data-changes230="${b.id}">What changed?</button>`:''}</div></div></div>`).join('')}`).join('')||'<div class="cv47-empty">No price list assigned.</div>'}</div><div id="cv47catalogSection">${(p.catalogs||[]).length?`<div class="section-title" style="margin-top:22px">Product catalogs</div><div class="cv47-catgrid">${p.catalogs.map(c=>`<div class="card" data-cat-card="${c.id}"><div class="bd"><div class="cv47-booktag">${esc(c.category||'Other')}</div><b>${esc(c.title||c.name||'Catalog')}</b><div class="note">PDF catalog · cached after first online open</div><div class="cv47-actions"><button class="btn ghost sm" data-cat-view="${c.id}">View</button><button class="btn primary sm" data-cat-down="${c.id}">Download PDF</button></div></div></div>`).join('')}</div>`:''}</div></div>`;e.querySelector('#cv46out').onclick=window.cloudViewerLogout46;let mode='search';const rr=()=>renderResults(e,p,mode);e.querySelector('#cv47q').oninput=()=>{mode='search';rr()};e.querySelector('#cv47book').onchange=rr;e.querySelector('#cv47catfilter').onchange=rr;e.querySelector('#cv47fav').onclick=()=>{mode='favorites';e.querySelector('#cv47q').value='';rr()};e.querySelector('#cv47recent').onclick=()=>{mode='recent';e.querySelector('#cv47q').value='';rr()};const sb=e.querySelector('#cv47switchbook');if(sb)sb.onchange=()=>{const id=sb.value,searchSel=e.querySelector('#cv47book');if(searchSel){searchSel.value=id;rr()}e.querySelectorAll('[data-book-card]').forEach(c=>c.style.display=!id||c.dataset.bookCard===id?'':'none')};const sc=e.querySelector('#cv47switchcat');if(sc)sc.onchange=()=>{const id=sc.value;e.querySelectorAll('[data-cat-card]').forEach(c=>c.style.display=!id||c.dataset.catCard===id?'':'none')};e.querySelectorAll('[data-pdf]').forEach(x=>x.onclick=()=>bookPdf((p.priceBooks||[]).find(b=>b.id===x.dataset.pdf)));e.querySelectorAll('[data-share]').forEach(x=>x.onclick=()=>bookShare((p.priceBooks||[]).find(b=>b.id===x.dataset.share)));e.querySelectorAll('[data-changes230]').forEach(x=>x.onclick=()=>changesDialog230((p.priceBooks||[]).find(b=>b.id===x.dataset.changes230),p));e.querySelectorAll('[data-cat-view]').forEach(x=>x.onclick=async()=>{const orig=x.textContent;x.disabled=true;x.textContent='Loading…';try{await viewCatalog(p.catalogs.find(c=>c.id===x.dataset.catView))}finally{x.disabled=false;x.textContent=orig}});e.querySelectorAll('[data-cat-down]').forEach(x=>x.onclick=async()=>{const orig=x.textContent;x.disabled=true;x.textContent='Downloading…';try{await downloadCatalog(p.catalogs.find(c=>c.id===x.dataset.catDown))}finally{x.disabled=false;x.textContent=orig}})}

  const authoritative=e=>!!(e&&e.__authoritative);
  async function closeViewerSession46({reason='',clearCache=false}={}){const cs=cloudSession;cloudSession=null;window.__restrictedFirebaseSession46=false;try{if(cs?.unsubProfile)cs.unsubProfile()}catch(e){}try{if(cs?.auth)await cs.auth.signOut()}catch(e){}try{if(cs?.app)await cs.app.delete()}catch(e){}if(clearCache&&cs?.loginName)try{localStorage.removeItem(CACHE_PREFIX+norm(cs.loginName))}catch(e){}const el=document.getElementById('cloudViewer46');if(el)el.remove();if(reason)toast(reason)}
  window.cloudViewerLogout46=()=>closeViewerSession46();
  ...
  async function activityCard230(v){if(!v||v.querySelector('#loginActivity230'))return;const a=api();if(!a?.auth?.currentUser||!a.db)return;const card=document.createElement('div');card.className='card';card.id='loginActivity230';card.style.marginTop='14px';card.innerHTML='<div class="hd"><h2>Device / login activity</h2></div><div class="bd"><div class="note">Loading recent restricted-user activity…</div></div>';v.appendChild(card);try{const snap=await a.db.collection('loginActivity').where('ownerUid','==',a.auth.currentUser.uid).get(),rows=[];snap.forEach(d=>rows.push({uid:d.id,...d.data()}));rows.sort((x,y)=>(y.lastLoginAt?.toMillis?.()||0)-(x.lastLoginAt?.toMillis?.()||0));card.querySelector('.bd').innerHTML=`<div class="tbl-wrap"><table><thead><tr><th>User</th><th>Device</th><th>Last login</th><th class="r">Logins</th></tr></thead><tbody>${rows.map(r=>{const u=(S.viewerUsers||[]).find(x=>x.cloudUid===r.uid),at=r.lastLoginAt?.toDate?.();return `<tr><td><b>${esc(u?.name||r.username||'Unknown')}</b><div class="metric-sub">${esc(r.role||u?.role||'')}</div></td><td>${esc(r.device||'—')}</td><td>${at?esc(at.toLocaleString('en-IN')):'—'}</td><td class="r">${+r.loginCount||0}</td></tr>`}).join('')||'<tr><td colspan="4" class="empty-mini">No cloud login activity yet.</td></tr>'}</tbody></table></div>`}catch(e){card.querySelector('.bd').innerHTML='<div class="note">Login activity will appear after the updated Firestore rules are published and a restricted user logs in.</div>'}}
  function enhance(){if(page!=='access45')return;const v=document.getElementById('view');if(!v)return;let card=v.querySelector('#secureAccess46');if(!card){const a=api();card=document.createElement('div');card.className='card';card.id='secureAccess46';card.style.marginTop='14px';card.innerHTML=`<div class="hd"><h2>Secure Firebase access</h2></div><div class="bd"><div class="private-note"><b>Security:</b> restricted users receive final rates and UOM alternatives only. Owner costing, discount formulas, inventory and settings are excluded. Offline viewer access expires after 24 hours or the user access-expiry date, whichever comes first.</div><div class="note" style="margin:10px 0">${a?.db?(a.auth?.currentUser?'Owner Firebase account is signed in.':'Firebase connected, owner not signed in.'):'Firebase is not configured on this device.'}</div><button class="btn ghost sm" data-act="init-secure46">Initialize security</button> <button class="btn ghost sm" data-act="publish-all46">Publish / update cloud users</button></div>`;v.appendChild(card)}if(!v.querySelector('#catalogLibrary47'))v.insertAdjacentHTML('beforeend',catalogLibraryHtml());v.querySelectorAll('[data-a45-edit]').forEach(btn=>{if(btn.parentElement.querySelector(`[data-cloud46="${btn.dataset.a45Edit}"]`))return;const id=btn.dataset.a45Edit,u=S.viewerUsers.find(x=>x.id===id),b=document.createElement('button');b.className='link';b.dataset.cloud46=id;b.textContent=u?.cloudUid?'sync cloud':'create cloud login';btn.insertAdjacentText('afterend',' · ');btn.after(b)});activityCard230(v)}

  function fingerprint(){
    try{
      const raw=JSON.stringify({products:(S.products||[]).map(p=>[p.id,p.code,p.name,p.price,p.mrp,p.priceUnit,p.priceListUnit,p.status,p.category,p.catalog,p.size,p.packing,p.barcode,p.netWt]),rules:S.rules||[],books:S.priceBooks||[],uom:S.uomMaster||[],slabs:S.quantityPriceSlabs||[],locks:S.customerRateLocks||[],scheduled:S.scheduledRates||[],parties:(S.parties||[]).map(p=>[p.id,p.name,p.archived]),settings:[S.settings?.firm,S.settings?.address,S.settings?.phone,S.settings?.email,S.settings?.gstin,S.settings?.wef,S.settings?.footer,S.settings?.currency,S.settings?.rounding],users:(S.viewerUsers||[]).map(u=>[u.id,u.name,u.role,u.pin,u.active!==false,u.accessExpiresOn||'',u.allowedPriceBookIds,u.allowedCatalogFileIds,u.rateNotice||null,u.announcement||null]),catalogs:(S.catalogFiles||[]).map(c=>[c.id,c.title,c.category,c.storagePath,c.filename,c.size]),brands:S.pdfBrandTemplates230||[]});
      let h1=0x811c9dc5,h2=0x9e3779b9;for(let i=0;i<raw.length;i++){const c=raw.charCodeAt(i);h1=Math.imul(h1^c,0x01000193);h2=Math.imul(h2+c,0x85ebca6b)^(h2>>>13)}return PUBLISH_REV+':'+raw.length+':'+(h1>>>0).toString(36)+':'+(h2>>>0).toString(36)
    }catch(e){return 'err:'+Date.now()}
  }

  const saveBeforeSecure=save;lastPublishFingerprint=S.settings?.viewerPublishedFingerprint228||'';
  async function publishPending(){
    const fp=pendingPublishFingerprint||fingerprint();
    if(fp===lastPublishFingerprint){pendingPublishFingerprint='';return true}
    const a=api();if(!navigator.onLine||!a?.auth?.currentUser){pendingPublishFingerprint=fp;return false}
    try{
      for(const u of (S.viewerUsers||[]).filter(x=>x.active!==false||x.cloudUid))await syncOne(u.id);
      lastPublishFingerprint=fp;pendingPublishFingerprint='';S.settings.viewerPublishedFingerprint228=fp;
      publishSuppress=true;try{saveBeforeSecure()}finally{publishSuppress=false}
      return true;
    }catch(e){pendingPublishFingerprint=fp;throw e}
  }
  function schedulePublish(delay=2200){
  ...
  document.addEventListener('click',async e=>{
    ...
    const a=e.target.closest('[data-act]');if(a?.dataset.act==='init-secure46'){e.preventDefault();window.initSecureAccess46()}if(a?.dataset.act==='publish-all46'){e.preventDefault();window.syncAllUsers46()}if(a?.dataset.act==='catalog-upload47'){e.preventDefault();catalogUploadDialog()}
  ...
  setTimeout(()=>render(),0);setTimeout(()=>schedulePublish(500),900);window.payloadFor46=payloadFor;window.showViewerPreview46=(p)=>show(p,false);window.getCloudSession233=()=>cloudSession;window.PriceManagerAPI={...(window.PriceManagerAPI||{}),version:'2.30 scheduled pricing + commercial workflow suite'};
})();
